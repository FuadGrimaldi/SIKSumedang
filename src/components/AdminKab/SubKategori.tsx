...
export default function SubKategoriManagerKab() {
  ...
  const [kecamatan, setKecamatan] = useState<
    { id: number; nama_kecamatan: string }[]
  >([]);
  ...
  const fetchSubKategoris = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/articles/kategori/sub-kategori/`);
  ...
  useEffect(() => {
    fetchSubKategoris();
    fetchKecamatan();
  }, [fetchSubKategoris, fetchKecamatan]);

  useEffect(() => {
    fetchKategoris();
  }, [kecamatanId, fetchKategoris]);
  ...
  const handleOpenEdit = (sub: SubKategoriArtikel) => {
    setEditData(sub);
    setKecamatanId(sub.kecamatan_id);
    setFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      kecamatan_id: Number(formData.get("kecamatan_id") as string),
      kategori_id: Number(formData.get("kategori_id") as string),
                ...
                {filteredSubKategoris.map((s, index) => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-700">
                      {s.kategori_article?.nama || "-"}
                    </td>
                    <td className="px-6 py-4 text-gray-700">
                      {s.profile_kecamatan?.nama_kecamatan || "-"}
                    </td>
