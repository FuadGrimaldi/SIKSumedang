...
export default function DashboardKab() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      const res2 = await fetch(`/api/articles/kategori/`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchAcara = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAcara(Array.isArray(data) ? data : []);
      }
    } catch (error) {
  ...
  const fetchInfografis = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("Infografis data:", data);
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Ringkasan Data dan Statistik</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6 rounded-lg shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100">Total Artikel dan Berita</p>
