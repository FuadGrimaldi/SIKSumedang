...
export default function ArtikelManagerKab() {
  ...
  const [kecamatanId, setKecamatanId] = useState<number | "">(""); // biar default kosong
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      const filtered = data.filter(
        (a: Article) =>
          a.kategori_article?.nama.toLocaleLowerCase() !== "berita" &&
  ...
  const fetchDropdowns = useCallback(async () => {
    ...
    try {
      ...
      const filterKategori = (await kategoriRes.json()).filter(
        (a: any) =>
          a.nama.toLocaleLowerCase() !== "berita" &&
  ...
  useEffect(() => {
    ...
    fetchKecamatan();
    fetchUser();
  }, [fetchArticles, fetchKecamatan, fetchUser]);
  ...
  const handleOpenView = (article: Article) => {
    ...
    setKecamatanId(article.kecamatan_id);
    setEditData(null);
    setFormOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-indigo-600 to-indigo-500 p-6 rounded-xl shadow-lg">
        <div>
          ...
          <p className="text-indigo-100 mt-1">
            Kelola data artikel di kecamatan ini
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 bg-white text-gray-700"
          />
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    name="user_id"
                    defaultValue={viewData?.user_id || editData?.user_id || ""}
                    disabled={!!viewData || submitting}
                    className="w-full px-4 py-2 border rounded-lg text-gray-700 bg-white"
                    required
                  >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    name="desa_id"
                    value={
                      viewData?.desa_id?.toString() ||
                      editData?.desa_id?.toString() ||
                      ""
                    }
                    onChange={(e) => {
                      if (editData)
                ...
                <div>
                  ...
                  <select
                    name="kategori_id"
                    value={
                      viewData?.kategori_id?.toString() ||
                      editData?.kategori_id?.toString() ||
                      ""
                    }
                    onChange={(e) =>
                      handleKategoriChange(Number(e.target.value))
                    }
                    disabled={!!viewData || submitting}
                    className="w-full px-4 py-2 border rounded-lg text-gray-700 bg-white"
                    required
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-700">Belum ada data</div>
      ) : (
