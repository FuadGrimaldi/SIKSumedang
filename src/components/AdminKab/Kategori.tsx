...
export default function KategoriManagerKab() {
  ...
  const fetchKategoris = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/articles/kategori/`);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      kecamatan_id: parseInt(formData.get("kecamatan_id") as string, 10),
                ...
                {filteredKategoris.map((k, index) => (
                  <tr key={k.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-700">
                      {k.profile_kecamatan?.nama_kecamatan}
                    </td>
